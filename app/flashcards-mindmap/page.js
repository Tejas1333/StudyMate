...
function transformJsonToD3TreeFormat(jsonData) {
  ...
  const buildNode = (name, childrenObj) => {
    ...
    if (
      childrenObj &&
      typeof childrenObj === "object" &&
      Object.keys(childrenObj).length > 0
    ) {
      node.children = Object.entries(childrenObj).map(
        ([childName, childValue]) => buildNode(childName, childValue),
      );
    }
    return node;
  };
...
const renderCustomNodeElement = ({ nodeDatum, toggleNode }) => (
  <g>
    <circle
      r={12}
      fill={nodeDatum.children ? "#20BEFF" : "#8c9eff"}
      onClick={toggleNode}
    />
    <text
      fill="#333"
  ...
  if (history.length === 0) {
    return (
      <p className="text-center text-gray-500 mt-8">
        No history found. Generate some study materials to get started!
      </p>
    );
  }
  ...
  return (
    <div className="space-y-4">
      {history.map((item) => (
        <div
          key={item._id}
          className="bg-white p-5 rounded-xl shadow-md border border-gray-200 flex items-center gap-4"
        >
          {editingTopicId === item._id ? (
            ...
            <>
              <input
                ...
                autoFocus
              />
              <button onClick={onSaveEdit} className="btn-primary">
                <FaCheck />
              </button>
...
export default function FlashcardsContent() {
  ...
  const currentFlashcards = selectedHistoryItem
    ? selectedHistoryItem.flashcards
    : flashcards;
    ...
    const saveStudyMaterials = async (materials) => {
      ...
      try {
        ...
        if (inputMethod === "topic") {
          finalTopic = topicName;
        }

        // 🔹 Text input
        else if (inputMethod === "plain_text") {
          finalTopic = "Text Flashcards";
        }

        // 🔹 PDF input
        else if (inputMethod === "pdf") {
          finalTopic =
            pdfFile?.name?.replace(/\.[^/.]+$/, "") || "PDF Flashcards";
        }

        const payload = {
          topic: finalTopic  || "Untitled Study Set",
        ...
        if (res.ok) {
          ...
          setError(
            `Failed to save: ${errorData.error || "Unknown server error"}`,
          );
        }
      } catch (err) {
  ...
  const handleDeleteCard = (indexToDelete) =>
    setEditedFlashcards(
      editedFlashcards.filter((_, index) => index !== indexToDelete),
    );
  ...
  const handleSaveChanges = async () => {
    ...
    try {
      ...
      setStudyHistory((prev) =>
        prev.map((item) =>
          item._id === documentId ? data.updatedMaterial : item,
        ),
      );
  ...
  const handleTopicUpdate = async () => {
    ...
    try {
      ...
      setStudyHistory((prev) =>
        prev.map((item) =>
          item._id === editingTopicId ? data.updatedMaterial : item,
        ),
      );
    ...
    <div className="max-w-screen-xl mx-auto py-12 px-4 sm:px-6 lg:px-8">
      <header className="text-center mb-8">
        ...
        <p className="text-xl text-gray-600">
          Generate, save, and customize your study materials.
        </p>
      </header>
      ...
      <div className="bg-white p-3 rounded-2xl shadow-lg mb-12 border border-gray-200 flex justify-center gap-4">
        <button
          ...
          className={`${view === "generator" ? "btn-primary" : "btn-secondary"}`}
        >
          <FaPencilAlt /> New Study Set
        </button>
        <button
          onClick={() => setView("history")}
          className={`${view.includes("history") ? "btn-primary" : "btn-secondary"}`}
        >
          <FaHistory /> History
        </button>
      </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-8">
          <div className="lg:col-span-2">
            <form
              ...
              className="bg-white p-6 rounded-2xl shadow-lg border border-gray-200 space-y-6 sticky top-24"
            >
              ...
              <div className="flex flex-col sm:flex-row gap-2 bg-gray-100 rounded-lg p-1">
                <button
                  ...
                  className={`w-full p-2 rounded-md font-semibold transition flex items-center justify-center ${inputMethod === "topic" ? "bg-white text-[#20BEFF] shadow" : "text-gray-600"}`}
                >
                ...
                <button
                  type="button"
                  onClick={() => setInputMethod("plain_text")}
                  className={`w-full p-2 rounded-md font-semibold transition flex items-center justify-center ${inputMethod === "plain_text" ? "bg-white text-[#20BEFF] shadow" : "text-gray-600"}`}
                >
                ...
                <button
                  ...
                  className={`w-full p-2 rounded-md font-semibold transition flex items-center justify-center ${inputMethod === "pdf" ? "bg-white text-[#20BEFF] shadow" : "text-gray-600"}`}
                >
              ...
              {inputMethod === "pdf" && (
                <input
                  ...
                  className="input-field file:bg-blue-50 file:text-[#20BEFF] hover:file:bg-blue-100"
                />
              )}
            ...
            <section className="bg-white p-6 rounded-2xl shadow-lg border border-gray-200">
              <div className="flex justify-between items-center mb-4">
                <h2 className="text-2xl font-bold text-gray-800 flex items-center">
                  <FaBookOpen className="mr-3 text-blue-500" />
                  Mind Map
                </h2>
              </div>
              <div
                ref={mindMapContainerRef}
                className="w-full h-[500px] bg-gray-50 rounded-lg flex items-center justify-center"
              >
                {loading && !mindMapTreeData && <p>Generating mind map...</p>}
                {!loading && !mindMapTreeData && (
            ...
            <section className="bg-white p-6 rounded-2xl shadow-lg border border-gray-200">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 perspective">
                {flashcards.map((card, index) => (
                  ...
                  >
                    <div
                      className={`flip-card-inner ${flippedCardIndices.has(index) ? "is-flipped" : ""}`}
                    >
              ...
              {editingTopicId === selectedHistoryItem._id ? (
                ...
                <div className="flex items-center gap-4">
                  <h2 className="text-3xl font-bold text-gray-800">
                    Flashcards for "{selectedHistoryItem.topic}"
                  </h2>
                  <button
                    onClick={() => handleTopicEditStart(selectedHistoryItem)}
              ...
              <div>
                {isEditing ? (
                  ...
                  <button
                    ...
                    className="px-4 py-2 bg-gray-200 text-[#20BEFF] rounded-lg font-semibold text-sm hover:bg-gray-300 disabled:opacity-50 flex items-center gap-2"
                  >
                    <FaPencilAlt /> Edit
                  </button>
                )}
